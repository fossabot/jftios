...
#define imageH 80 // 图片高度
#define imageW 80 // 图片宽度
#define kMaxColumn 3 // 每行显示数量
#define MaxImageCount 3 // 最多显示图片个数
#define deleImageWH 25 // 删除按钮的宽高
#define kAdeleImage @"closePlus.png" // 删除按钮图片
#define kAddImage @"addPlus.png" // 添加按钮图片
...
- (UIButton *)createButtonWithImage:(id)imageNameOrImage andSeletor : (SEL)selector
{
    ...
    NSLog(@"-----104---%f", addBtn.frame.size.width);
...
- (void)layoutSubviews
{
    ...
    //(self.frame.size.width - maxColumn * btnW) / (count + 1)
    CGFloat marginX = 10;
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    UIImage* image1=(UIImage*)[info objectForKey:@"UIImagePickerControllerEditedImage"];
    NSLog(@"200获取照片------%@", image1);
    if (editTag == -1) {
        // 创建一个新的控件
